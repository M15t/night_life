...
class CustomClearableFileInputWidget(ClearableFileInput):
    ...
    def render(self, name, value, attrs=None):
        ...
        data['input'] = super(CustomClearableFileInputWidget, self).render(name, value, attrs)

        if hasattr(value, 'url'):
            ext = str(value).split('.')[-1]
            if ext in ['jpg', 'png', 'svg']:
                data['url'] = conditional_escape(value.url)
                template = '<div class="thumbnail" style="margin-bottom:10px"><a href="%(url)s" data-popup="lightbox">' + '<img src="%(url)s">' + '</a></div>' + '%(input)s'
            elif ext in ['mp3']:
                data['url'] = conditional_escape(value.url)
                template = '<audio class ="w-full" preload="none" controls ><source src="%(url)s" /></audio>' + '%(input)s'
            elif ext in ['mp4']:
                data['url'] = conditional_escape(value.url)
            ...
            if not self.is_required:
                ...
                data['clear'] = CheckboxInput().render(checkbox_name, False, attrs={'id': checkbox_id})
...
class AdminImageWidget(ClearableFileInput):
    def render(self, name, value, attrs=None):
        ...
        if value and hasattr(value, "url"):
            ...
            data['initial'] = (
                '<img src="%s" alt="%s" style="max-width: 100px; max-height: 100px; border-radius: 5px;" /><br/>' % (escape(value.url), escape(force_unicode(value)))
            )
            if not self.is_required:
                checkbox_name = self.clear_checkbox_name(name)
                checkbox_id = self.clear_checkbox_id(checkbox_name)
